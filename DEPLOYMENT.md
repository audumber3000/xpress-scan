...
### 7. Sync Project Files (from your local Mac)
```bash
sshpass -p '<PASSWORD>' rsync -avz --progress \
  --exclude='venv/' --exclude='.venv/' --exclude='__pycache__/' \
  --exclude='*.pyc' --exclude='.env' --exclude='node_modules/' \
  --exclude='.git/' --exclude='uploads/' --exclude='build/' \
  --exclude='dist/' --exclude='*.log' --exclude='.DS_Store' \
  backend nexus-service docker-compose.prod.yml deploy.sh .env.production \
...
sshpass -p '<PASSWORD>' rsync -avz \
  ...
  --exclude='*.pyc' --exclude='.env' --exclude='node_modules/' \
  --exclude='.git/' --exclude='uploads/' --exclude='build/' \
  backend nexus-service docker-compose.prod.yml \
  root@178.104.132.255:/root/molarplus/

# 2. Rebuild and restart on server
sshpass -p '<PASSWORD>' ssh root@178.104.132.255 \
  "cd /root/molarplus && docker compose -f docker-compose.prod.yml --env-file .env.production up -d --build"
```
