...
class SupportBlogPost(Base):
    """AI Blog Studio drafts. Generated by Claude, reviewed/edited by an admin,
    then published to Sanity (which powers the marketing-site blog)."""
    __tablename__ = 'support_blog_posts'
    id = Column(Integer, primary_key=True)
    topic = Column(String, nullable=True)            # the prompt/topic used to generate
    ...
    sanity_id = Column(String, nullable=True)        # Sanity document _id once published
    published_url = Column(String, nullable=True)    # live blog URL
    model_used = Column(String, nullable=True)       # which Claude model generated it
