...
const VerifyContactStep = ({ phone: initialPhone, email: initialEmail, onVerified }) => {
  ...
  // Guards React 18's double-invoked effect in development, which would
  // otherwise fire two sends and burn the first code immediately.
  ...
  const send = useCallback(async (to = { phone, email }) => {
    ...
    setError('');
    try {
      const res = await api.post('/security/signup-otp/send', { phone: to.phone, email: to.email });
      setReached(res.reached || []);
      setDevEcho(!!res.dev_echo);
      setFailed(Object.entries(res.delivery || {}).filter(([, r]) => !r.sent).map(([ch]) => ch));
      setBlocked(false);
  ...
  const verify = async () => {
    ...
    try {
      ...
      setError(getFriendlyErrorMessage(err, 'That code did not work.'));
    } finally {
      setVerifying(false);
    }
  };
  ...
  const saveContacts = async () => {
    ...
    setEmail(next.email);
    setEditing(false);
    // A corrected typo resends immediately: the 45s cooldown is per target, and
    // the target just changed.
    setCooldown(0);
    await send(next);
  };
      ...
      {!editing && (
        <>
          ...
          <div>
            ...
            />
            <p className="mt-1.5 text-xs text-gray-400">It expires in 5 minutes.</p>
          </div>
          ...
          {/* Nothing reached them at all. This is the state that would otherwise
              be a dead end, so it leads with a human rather than a retry. */}
          {blocked && (
            <div className="rounded-xl border border-red-200 bg-red-50 p-4">
              <p className="text-sm font-semibold text-red-700">We could not reach you on either channel</p>
