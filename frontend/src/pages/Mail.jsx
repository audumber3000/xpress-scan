  ...
  if (!isGmailConnected) {
    return (
      <div className="p-6 md:p-8 bg-gray-50 min-h-full">
        <div className="mt-10 bg-white border border-gray-100 shadow-sm rounded-2xl p-12 text-center max-w-3xl mx-auto flex flex-col items-center">
          <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mb-6 shadow-inner">
            <GoogleIcon className="w-10 h-10" />
          </div>
          <h2 className="text-2xl font-bold text-gray-900 mb-3">Connect Your Gmail Account</h2>
          <p className="text-gray-500 max-w-lg mb-8 text-sm leading-relaxed">
            Link your Google account to read and send emails directly from your dashboard. Replies, threads and patient communication all in one place.
          </p>
          <button
            onClick={handleGmailLogin}
            className="flex items-center gap-3 bg-white border-2 border-slate-200 text-gray-800 px-6 py-3 rounded-xl font-bold shadow-sm hover:bg-slate-50 transition-colors"
          >
            <GoogleIcon className="w-5 h-5" />
            Sign in with Google
          </button>
          <p className="mt-5 text-xs text-gray-400">Secure OAuth Integration via Google Cloud APIs</p>
        </div>
      </div>
    );
  }
