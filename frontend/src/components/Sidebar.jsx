...
import { ChevronDown, ChevronRight, HelpCircle, X, Send } from "lucide-react";
...
const Sidebar = ({ isMobileOpen, onMobileClose, isCollapsed, onCollapseChange }) => {
  ...
  const [helpForm, setHelpForm] = useState({ title: '', description: '', category: 'setup', priority: 'normal' });

  const handleSubmitTicket = async () => {
    if (!helpForm.title.trim()) return;
    setHelpSubmitting(true);
    try {
      await api.post('/support-tickets', helpForm);
      setHelpModalOpen(false);
      setHelpForm({ title: '', description: '', category: 'setup', priority: 'normal' });
    ...
    <>
      ...
      {helpModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60" onClick={() => setHelpModalOpen(false)} />
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 z-10">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-lg font-bold text-gray-900">Need Help?</h2>
                <p className="text-xs text-gray-500 mt-0.5">Our support team will respond shortly</p>
              </div>
              <button onClick={() => setHelpModalOpen(false)} className="p-2 rounded-xl hover:bg-gray-100 text-gray-400">
                <X size={18} />
              </button>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setHelpForm(f => ({ ...f, category: e.target.value }))}
                    className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#2a276e]/20 focus:border-[#2a276e]"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setHelpForm(f => ({ ...f, priority: e.target.value }))}
                    className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#2a276e]/20 focus:border-[#2a276e]"
                  >
            ...
            <div className="flex justify-end gap-2 mt-5">
              <button onClick={() => setHelpModalOpen(false)} className="px-4 py-2.5 text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-xl">
                Cancel
              </button>
              <button
                onClick={handleSubmitTicket}
                disabled={helpSubmitting || !helpForm.title.trim()}
