                ...
                <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
                    <div className="space-y-3">
                        ...
                        <div className="space-y-2" ref={suggestionRef}>
                            {items.map((item, i) => (
                                <div key={i} className="rounded-xl border border-gray-200 bg-white px-3 py-2.5">
                                    <div className="flex items-start gap-2.5">
                                        <span className="mt-2 w-4 text-center text-xs font-bold text-gray-300 select-none">{i + 1}</span>
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="relative">
                                                <input
                                                    ...
                                                    className="w-full border-0 border-b border-gray-200 px-0 py-1 text-sm font-semibold text-gray-900 bg-transparent focus:outline-none focus:border-[#2a276e] placeholder:font-normal placeholder:text-gray-400"
                                                />
                                                {showSuggestions === i && filteredMeds.length > 0 && (
                                                    <div className="absolute top-full left-0 z-[80] w-full bg-white border border-gray-200 rounded-xl shadow-xl mt-1 max-h-44 overflow-y-auto">
                                                        {filteredMeds.map((med, mi) => (
                                                            <button key={mi} onClick={() => selectMedication(i, med)}
                                                                className="w-full text-left px-4 py-2.5 text-sm hover:bg-[#2a276e]/5 flex items-center justify-between gap-2">
                                                                <span className="font-medium text-gray-800 truncate">{med.name}</span>
                                                                <span className="text-xs text-gray-400 flex-shrink-0">{med.dosage} · {med.duration}</span>
                                                            </button>
                                                        ))}
                                                    </div>
                                                )}
                        ...
                        <button onClick={addItem}
                            className="w-full py-2.5 rounded-xl border border-dashed border-gray-300 text-sm font-semibold text-[#2a276e] hover:border-[#2a276e] hover:bg-[#2a276e]/5 transition-all flex items-center justify-center gap-1.5">
