...
const PatientPrescriptions = ({ patientId, patientPhone, visits = [], hideHeader = false }) => {
    ...
    const [deletingRx, setDeletingRx] = useState(false); // for detail drawer
                    ...
                    <div className="px-6 py-4 border-t border-gray-100 space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            ...
                            <button onClick={handleDrawerDelete}
                                disabled={deletingRx}
                                className="py-2.5 bg-red-50 text-red-500 rounded-xl font-semibold text-sm hover:bg-red-100 transition-all border border-red-100 inline-flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed">
                                {deletingRx ? 'Deleting' : 'Delete'}
                                {deletingRx && <Spinner className="w-3.5 h-3.5" />}
                            </button>
                        </div>
                    </div>
                </div>
