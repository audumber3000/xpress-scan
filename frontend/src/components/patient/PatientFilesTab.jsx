...
const fileKind = (file) => {
  ...
  const t = (file.file_type || file.image_type || '').toLowerCase();
  if (/\.(dcm|dicom)$/.test(u) || t.includes('xray') || t.includes('x-ray')) return { label: 'X-ray', cls: 'bg-indigo-50 text-indigo-600' };
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between gap-3">
                ...
                <label className={`flex items-center gap-2 px-5 py-2.5 rounded-lg font-semibold text-sm transition-colors cursor-pointer shadow-sm flex-shrink-0 ${uploading ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-[#2a276e] text-white hover:bg-[#1a1548]'}`}>
                    ...
                    <input type="file" className="hidden" onChange={(e) => uploadFile(e.target.files[0])} disabled={uploading} />
                </label>
            </div>
                            ...
                            return (
                                <div key={file.id} className="group bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-xl transition-all">
                                    <div className="aspect-video bg-gray-900 flex items-center justify-center relative">
                                        {isImage(file) && src ? (
                                            <img src={src} alt={file.file_name} className="w-full h-full object-cover" loading="lazy" />
                                        ) : (
                                            <svg className="w-12 h-12 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                            </svg>
                                        )}
                                        <span className={`absolute top-2 left-2 text-[10px] font-bold px-2 py-0.5 rounded-full ${kind.cls}`}>{kind.label}</span>
                                    ...
                                    <div className="p-4">
                                        ...
                                        <div className="flex items-center justify-between mt-2 text-xs text-gray-400">
                                            <span className="font-semibold">{new Date(file.created_at || file.capture_date).toLocaleDateString()}</span>
                                            {file.uploader_name && <span className="italic truncate ml-2">by {file.uploader_name}</span>}
                                        </div>
                                    </div>
                                </div>
                            );
