...
import { apiService, Appointment } from '../../services/api/apiService';
...
export const AppointmentsScreen: React.FC<AppointmentsScreenProps> = ({ navigation }) => {
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    try {
      console.log('🔄 [APPOINTMENTS] Refreshing appointments...');
      const dateStr = selectedDate.toISOString().split('T')[0];
      const data = await apiService.getAppointments(dateStr, viewMode.toLowerCase() as 'week' | 'month');
      setAppointments(data);
      console.log('✅ [APPOINTMENTS] Refreshed', data.length, 'appointments');
    } catch (err: any) {
