"""Classic invoice variant — the original layout.

This is the byte-identical port of the previous `invoice_pdf_engine.py` rendering
logic. The Phase 0 golden test pins this output, so any change here must be a
deliberate redesign, not a drift.
"""
import datetime
...
def render_invoice(invoice, clinic, config=None) -> str:
    """Render the Classic invoice variant. Public API: stable, golden-tested."""
    # Primary colour: config → clinic attribute → smart default. Sanitise for CSS.
