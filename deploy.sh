...
# claiming a send.
run_migration "treatment_summary_pref" "INSERT INTO notification_preferences (clinic_id, event_type, channels, is_enabled) SELECT c.id, 'treatment_summary', '[\"whatsapp\"]'::json, FALSE FROM clinics c WHERE NOT EXISTS (SELECT 1 FROM notification_preferences p WHERE p.clinic_id = c.id AND p.event_type = 'treatment_summary')"
